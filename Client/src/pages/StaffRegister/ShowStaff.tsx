...
import { useGetAllAdminMutation } from "../../Slice/ApiSclice/AdminApi";
...
type Prop = {
  updateStart:(username:string, email: string,password:string , _id: string)=>void
  delteBtn:(val:string)=>void
}

function ShowStaff({updateStart,delteBtn}:Prop) {
  const header = ["Admin ID", "Name", "email", 'role',"action"];
  const [getall] = useGetAllAdminMutation()
    ...
    const [tableID,setTabelId]=useState('')
      ...
      const confirm=(No:string,Id:string)=>{
         setTableNo(No)
         SetConfirmPop(true)
         setTabelId(Id)
      }
    ...
    if (Array.isArray(res.data)) {
      ...
      setTabelData([]); // fallback to empty array
    }
  };
  data();
      }, [getall, tableData])
                ...
                pageData.map((data, index) => {
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">{data.username}</td>
<td scope="row" className="px-6 py-4 text-center">
                        {data.email}
                      </td>
                     <td scope="row" className="px-6 py-4 text-center">
                      ...
                      <td className="px-6 py-2 text-center">
                        <div className="flex gap-2 items-center justify-center ">
                          ...
                          <FontAwesomeIcon
                            icon={faTrash}
                            onClick={() => confirm(data.username, data._id)}
          ...
          {ConformPop && (
            <div className="fixed top-30 left-2/7 rounded-md lg:left-2/5 bg-gray-500  dark:bg-gray-900 p-5">
              <div className="flex items-center ">
                ...
                />
                <h1 className="ml-3 text-2xl font-bold">Delete Admin</h1>
              </div>
